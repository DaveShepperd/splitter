...
#endif
#ifndef MAX_BYTESKIP
#define MAX_BYTESKIP 7
#endif
#define XMK_STRING(x) MK_STRING(x)
...
static int help_em(const char *name)
{
			fprintf(stderr, "Usage: %s [opts]\n"
					...
					"-b, --byteskip=n    - Set the number of input bytes to skip while copying to output. (Can only be 0 <= n <= " XMK_STRING(MAX_BYTESKIP) "; see NOTE below.)\n"
					...
					"-V, --version       - Display the current version (to stderr)\n"
					"-h,-?, --help       - This message\n"
					"\n"
					...
					"%s --inskip=0x201 --outskip=0x80 --byteskip=1 < foo > bar\n"
					"# etc.\n"
					,name
					,name
					,name
...
{
	U32 inSkip=0, outSkip=0, skipInAmt, skipOutAmt, totalOutCount=0, outWritten=0;
	int opt, byteSkip=0;
	...
	int verbose=0, version=0;

	while ( (opt = getopt_long(argc, argv, "i:o:s:S:b:c:f:h?vV",
					long_options, &option_index)) != -1 )
	{
		...
		case 'b':
			...
			if ( !endp || *endp || byteSkip < 0 || byteSkip > MAX_BYTESKIP )
			{
				fprintf(stderr,"Bad --byteskip parameter: '%s'. Must be 0 <= n <= " XMK_STRING(MAX_BYTESKIP) "\n", optarg);
				return 1;
			}
			break;
		case OPT_COUNT:
	...
	{
		fprintf(stderr,"Sorry, On Windows systems (at least under MSYS2), the stdin stdout thing doesn't work. So you have to specify in/out filenames with --infile and --outfile\n");
		return 1;
	}
#endif
	if ( outFile )
	...
	skipInAmt = inSkip;
	++byteSkip;			/* Byteskip says how many bytes to skip over */
	while ( skipInAmt )
	{
		limit = sizeof(buff);
		if ( limit > skipInAmt )
		...
		skipInAmt -= len;
		if ( verbose )
			fprintf(stderr,"Skipped 0x%0X bytes of input. 0x%0lX remaining.\n", len, skipInAmt);
	}
	...
	while ( skipOutAmt )
	{
		...
		skipOutAmt -= len;
		if ( verbose )
			fprintf(stderr,"Skipped 0x%0X bytes of output. 0x%0lX bytes reminaing.\n", len, skipOutAmt);
	}
	while ( !totalOutCount || (outWritten < totalOutCount) )
	{
		...
		if ( len == 0 )
			break;
		if ( verbose )
			fprintf(stderr,"Read 0x%0X bytes from input.\n", len);
		if ( byteSkip != 0 )
		{
			int ii;
		...
		outWritten += len;
		if ( verbose )
			fprintf(stderr,"Wrote 0x%0X bytes to output. Total written so far: 0x%0lX\n", len, outWritten);
	}
	if ( totalOutCount && outWritten < totalOutCount )
	{
		...
		while ( outWritten < totalOutCount )
		{
			...
			outWritten += len;
			if ( verbose )
				fprintf(stderr,"Wrote 0x%0X bytes of padding to output. Total written so far: 0x%0lX\n", len, outWritten);
		}
	}
	return 0;
}
